...
export const realtimeEventSchema = z.discriminatedUnion("type", [
  ...
  z.object({
    type: z.literal("phrase_trigger"),
    phraseText: z.string(),
    sceneId: z.number(),
  }),
  z.object({
    type: z.literal("scene1_complete"),
  }),
  z.object({
    type: z.literal("volume_change"),
    volume: z.number().min(0).max(1),
  }),
  z.object({
    type: z.literal("client_identify"),
    role: z.enum(["remote", "stage"]),
  }),
  z.object({
...
// WebSocket message types (legacy - keeping for compatibility)
